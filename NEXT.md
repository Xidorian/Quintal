...
- [x] **QT-034 — Idealista filter-URL format discovered (2026-07-19).** From the live UI: `…/faro-distrito/com-preco-max_1500,t2,t3,t4-t5/`. The price token was always right; the bug was the bedroom token — Idealista has **no standalone `t4`** (a bare `t4` soft-404s the whole URL); the top option is **T4+ = `t4-t5`**. Adapter now emits `t{lo..3}` then `t4-t5` when `hi ≥ 4` → generates the exact working URL. Future Idealista pulls come pre-filtered (≤€1500, T2–T4+), no more paging through the full 1,697. 2 tests.
...
- [x] **QT-033 — Ingest ergonomics (built 2026-07-19).** Versioned `collect/extract.js` — the per-site card selectors + accumulate/download helpers in one maintained file, replacing the re-pasted-per-page snippet. Flow: inject → `quintalReset(site)` (page 1) → `quintalExtract(site)` (each page → `localStorage`) → `quintalDownload(site)` (fresh tab → `~/Downloads`) → `--ingest`. Selectors validated live 2026-07-19. Fix selectors there when a portal moves them.
- [x] **QT-030 — Valuation-outlier robustness (built 2026-07-19).** Hedonic ridge now fits only on log(rents) within `VALUATION_FIT_MAD_K` (3.5) MADs of the median, so sale/trespasse leaks + luxury villas don't distort the coefficients; every listing is still scored. MAD-based (not fixed-percentile) so a lone extreme point is caught even in a small pool, and it won't over-trim a thin pool. Real-pool effect: ~18 listings moved out of a false-undervalued bias (undervalued 224→206, overpriced 186→204). 3 tests. NB the QT-029 price fix was the bigger de-distorter; this handles the residual real-outlier skew.
- [x] **QT-031 — Screening gap fixed (2026-07-19).** Replaced the 4 literal month-spans with a bounded regex (`setembro|outubro … a[te] … maio|junho|julho`, capped at 15/10 non-period chars) that tolerates a year between the months ("Setembro 2025 a Junho 2026") without spanning sentences. Catches 27 seasonal lets in the current pool; cross-sentence negative test guards over-matching. 3 tests.
...
- [x] **QT-035 — OpenRouteService routed walk-times (built 2026-07-19).** ORS key in local `.env`. `BeachEnricher` keeps the nearest beach's coords and routes origin→beach (foot-walking); results cached by rounded endpoints in `enrichment_cache.json`. `walk_minutes` reads a cached route **even without a key**, so the 140 routes computed once locally serve the hosted app (no ORS key on Streamlit). Falls back to the straight-line estimate on any ORS failure; 40/min throttle. Routed times are +5.8 min median longer than crow-flies (real paths bend around buildings/water) — a genuine gain for the 20%-weighted beach-walk score. 4 tests.
...
- [x] **Idealista thumbnails (done 2026-07-19)** — captured card `<img>` src during the fresh browser collection (QT-028 wiring); 242 new photos downloaded incl. Idealista. Older idealista records (pre-collection) still lack one until re-collected.
- [x] **QT-028 — Wire `image_url` into the extractors** (built 2026-07-19). `ExtractedRow.image_url` → `row_to_raw` → `Listing.photos[0]`; `photos.fetch_one` prefers a captured url (downloads it directly), falling back to the detail-page og:image only when absent. Future pulls get thumbnails without the per-listing fetch — **and it's the Idealista-thumbnail route** (see above): capture card `<img>` src during the browser pull. No effect on current data (nothing captured yet). 3 tests.
- [x] **QT-032 — Photo-hash dedup (built 2026-07-19).** `photo_hash.dhash` (difference hash) + a guarded second dedup pass over the attribute-canonicals, bucketed by bedrooms, merging only when thumbnails match (Hamming ≤6) AND bedrooms + price (±10%) corroborate. Catches cross-source dupes the attribute rule misses because the concelho was parsed differently (Almancil vs Loulé); the price guard rejects two different flats sharing a generic photo. **18 extra dupes** on the current pool. Pillow declared. 5 tests. (NB: Idealista thumbnails are `/blur/` previews, so idealista↔imovirtual photo matches are partial — most catches are same-source or clear-photo cross-source.)
